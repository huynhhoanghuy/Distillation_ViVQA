...
def main(args):
    ...
    def loss_fn_kd_intermediate(outputs, labels, teacher_outputs):
        ...
        total_loss = (1-alpha)*loss_CE(outputs, labels)  + \
                     alpha* loss_KD(F.log_softmax(outputs, dim=1),F.softmax(teacher_outputs, dim=1))
            ...
            with torch.set_grad_enabled(phase == 'train'):        
                ...
                with tqdm(dataloaders[phase], unit=' batch', total=len(dataloaders[phase])) as tq_data:
                    for i, data in enumerate(tq_data):
                        ...
                        output = student_model.forward(img, question)
                        output = student_model.classify(output)


                        if phase == 'train':
                            teacher_output = teacher_model.forward(img, question)
                            teacher_output = teacher_model.classify(teacher_output)
    
                            # Compute the loss and accuracy
                            loss = loss_fn_kd_intermediate(output, label, teacher_output)
