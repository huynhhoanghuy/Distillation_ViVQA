...
def main(args):
    ...
    student_model = replaceResNetViT(args, mobiNetCNN,mobiNetViT)
    
    student_model.to(device)
    

    teacher_model = getattr(base_model, constructor)(args)
    teacher_model.load_state_dict(torch.load("/data/huy.hhoang/distill_student_list_teacher/experiments/ViVQA/GuidedAtt_vit_resnet34_phobert_11_04_2023__11_31_10.pt"))
    teacher_model.eval()
    ...
    loss_CE = torch.nn.CrossEntropyLoss(reduction='sum', label_smoothing=args.label_smooth)
    ...
    def loss_fn_kd_intermediate(outputs, labels, teacher_outputs, student_vit, teacher_vit, student_cnn, teacher_cnn):
        ...
        #                          F.softmax(teacher_outputs/T, dim=1)) * ((alpha)/3 * T * T) + \
        #           feat_kd_loss(F.log_softmax(student_vit), F.softmax(teacher_vit))* ((alpha)/3) + \
        #           feat_kd_loss(F.log_softmax(student_cnn), F.softmax(teacher_cnn))* ((alpha)/3)+ \
        #           F.cross_entropy(outputs, labels) * (1. - alpha)

        total_loss = (1-alpha)* loss_CE(outputs, labels)  + \
                     (alpha*1/5)* loss_KD(F.log_softmax(outputs, dim=1),F.softmax(teacher_outputs, dim=1)) +\
                     (alpha*2/5)* loss_KD(F.log_softmax(student_vit, dim=1), F.softmax(teacher_vit, dim=1)) +\
                     (alpha*2/5)* loss_KD(F.log_softmax(student_cnn, dim=1), F.softmax(teacher_cnn, dim=1))
            ...
            with torch.set_grad_enabled(phase == 'train'):        
                ...
                with tqdm(dataloaders[phase], unit=' batch', total=len(dataloaders[phase])) as tq_data:
                    for i, data in enumerate(tq_data):
                        ...
                        output, student_vit, student_cnn = student_model.forward(img, question)
                        output = student_model.classify(output)


                        if phase == 'train':
                            teacher_output, teacher_vit, teacher_cnn = teacher_model.forward(img, question)
                            teacher_output = teacher_model.classify(teacher_output)
    
                            # Compute the loss and accuracy
                            loss = loss_fn_kd_intermediate(output, label, teacher_output, student_vit, teacher_vit, student_cnn, teacher_cnn)
